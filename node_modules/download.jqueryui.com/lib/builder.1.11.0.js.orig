...
=======
			data: banner( jqueryUi.pkg, files.i18nFiles.map( path.basename ), { minify: true }) // FIXME + stripBanner( files.min( this.bundleI18n[ 0 ] ) )
>>>>>>> (WIP) Instead of changing all DownloadBuilder to expect an async build, changed rjs to return a Stream, which will be handled appropriately in Packer#zipTo and will have its async aspect taken care.
		});
	} else {
...
=======
		data: banner( jqueryUi.pkg, jsComponentFileNames, { minify: true } ) /* FIXME + minify this.bundleJs[0].data */
>>>>>>> (WIP) Instead of changing all DownloadBuilder to expect an async build, changed rjs to return a Stream, which will be handled appropriately in Packer#zipTo and will have its async aspect taken care.
	});
